#include "Head.h"
#include "CS2_SDK.h"
const string Rensen_ReleaseDate = "[2024-05-05 18:50]";//程序发布日期
const float Rensen_Version = 3.62;//程序版本
	...
	while (true)
	{
		...
		if (!GUI_VAR.Window_Move() && Menu_Open)//不在移动窗口时绘制GUI
		{
			...
			else if (UI_Panel == 1)//Visual
			{
				...
				GUI_VAR.GUI_Slider<int, class CLASS_Rensen_Menu_22>(Block_ESP, 16, "Draw delay", 1, 30, UI_Visual_ESP_RenderSleep, "ms");
			...
			else if (UI_Panel == 4)//Debug
			{
				...
				GUI_VAR.GUI_InputText<class CLASS_Rensen_Menu_36>({ Block_DebugControl.x - 15,Block_DebugControl.y }, 2, Debug_Control_Var::SystemCommand, "Command");
...
void Thread_Funtion_PlayerESP() noexcept//功能线程: 透视和一些视觉杂项
{
	...
	while (true)
	{
		Sleep(UI_Visual_ESP_RenderSleep);//降低CPU占用
		...
		if (CS2_HWND && (Menu_Open || Global_IsShowWindow))//当CS窗口在最前端 && 菜单在最前端
		{
			...
			if (UI_Legit_Aimbot && UI_Legit_Armory_ShowAimbotRange && Global_LocalPlayer.Health())//Aimbot Range 自瞄范围绘制
			{
				...
				ESP_Paint.RenderA_GradientCircle(CS_Scr_Res.r / 2, CS_Scr_Res.g / 2, Variable::Animation<class Render_Aimbot_Range_Animation>(Circle_Range, 1.5), { 0,0,0,0 }, GUI_IO.GUIColor.D_Alpha(80), 0.95);
			}
